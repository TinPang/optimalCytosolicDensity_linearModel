arr_occupancy = 0.005:0.005:0.995;
arr_ratioS = 10.^(-3:0.001:-0.001);
...
ccc = bbb;
ccc_rate_rxn = bbb_rate_rxn;

figure(1);
% plots flux vs occupancy curve, i.e. Fig 3 in the manuscript
subplot(2,1,1);
hold on; plot(aaa(1:end),ccc_rate_rxn(:,1)/max(ccc_rate_rxn(:,1)),['b',arr_marker{Nprotein/20}]);
hold on; plot(aaa(1:end),ccc_rate_rxn(:,2)/max(ccc_rate_rxn(:,2)),['r',arr_marker{Nprotein/20}]);
...
xlabel('occupancy \rho');
ylabel('relative flux per dry mass $\frac{\nu}{\nu_{max}}$','Interpreter','latex');
KM0 = 130*602;

% plots the K_M^*/K_M^0 vs occupancy curve, i.e. Fig 7 in the manuscript
figure(2); 
hold on; semilogy(aaa(1:end),ccc(:,3)/KM0,['b',arr_marker{Nprotein/20}]);
hold on; semilogy(aaa(1:end),ccc(:,4)/KM0,['r',arr_marker{Nprotein/20}]);
xlabel('occupancy \rho')
ylabel('K_M^* / K_M^0')
