...
%arr_KM = KM0 .* ones(length(arr_occupancy),length(arr_ratio_s));
%arr_log_Gamma = zeros(length(arr_occupancy),length(arr_ratio_s));
for q = 1:length(arr_occupancy)
  %N_misc = arr_N_misc(q);
  rrho = arr_occupancy(q);
  for r = 1:length(arr_ratio_s)
      rho_s = arr_occupancy(q) * arr_ratio_s(r);
      rho_E = arr_occupancy(q) * (1-arr_ratio_s(r));
      ss = rho_s * rho_vol / v_s / N_protein;
      EE = rho_E * rho_vol / v_E / N_protein;
      complex_D = EE*ss/(KM0 + ss) / (1-rrho);
...
for q = 1:length(arr_occupancy)
  ...
  for r = 1:length(arr_ratio_s)
      ...
      ss = rho_s * rho_vol / v_s / N_protein;
      EE = rho_E * rho_vol / v_E / N_protein;
      complex_D = EE*ss/(KM0 + ss) / (1-rrho) * exp(-5.8*rrho);
      arr_complex(q,r) = complex_D;
      %arr_KM(q,r) = 1/cur_1overKM;    
  end  
end
