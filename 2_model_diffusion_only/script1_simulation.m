...
for q = 1:length(arr_occupancy)
  for r = 1:length(arr_ratio_s)
      [cur_1overKM,complex_D,log_Gamma] = f_calc_d_without_ts(rho_vol,ttheta,arr_HSV_SED,rho_vol*arr_occupancy(q),arr_ratio_s(r),N_protein,KM0);
...
for q = 1:length(arr_occupancy)
  for r = 1:length(arr_ratio_s)
      [cur_1overKM,complex_D,log_Gamma] = f_calc_d_without_ts(rho_vol,ttheta,arr_HSV_SED,rho_vol*arr_occupancy(q),arr_ratio_s(r),N_protein,KM0); % f_calc_d_without_ts calculates the scaling factor g(r_s), assuming diffusion as the only factor limiting the rate of a reaction    
      arr_complex(q,r) = complex_D;
      arr_KM(q,r) = 1/cur_1overKM;    
  end  
end
