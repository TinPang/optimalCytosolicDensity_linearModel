...
for q = 1:length(arr_occupancy)
  for r = 1:length(arr_ratio_s)
      [cur_1overKM,complex_D,log_Gamma] = f_calc_d_without_G(rho_vol,ttheta,arr_HSV_SED,rho_vol*arr_occupancy(q),arr_ratio_s(r),N_protein,KM0); % f_calc_d_without_G calculates the scaling factor g(r_s), assuming transition state limit
