function [cur_1overKM,complex_D,log_Gamma] = f_calc_d_without_G(rho_vol,ttheta,arr_HSV_SED,vol_occ,ratio_s,N_protein,KM0)
...
%disp([avg_V log_Gamma]);
    ...
    termm_denominator = (1+ttheta)*exp(-1*0*avg_V);
    termm_numerator = 1;
  else
    ...
    termm_denominator = (1+ttheta)*exp(log_Gamma-0*avg_V);
    termm_numerator = exp(log_Gamma) + ttheta*exp(-1*0*avg_V);
  end
